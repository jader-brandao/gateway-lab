...
<template>
    <aside
        :class="[
            'fixed left-0 top-0 z-[99999] flex h-screen flex-col border-r border-zinc-200/80 bg-zinc-50 transition-all duration-300 ease-in-out dark:border-zinc-700/50 dark:bg-zinc-800',
            {
    ...
    >
        ...
        <nav class="min-h-0 flex-1 overflow-y-auto overflow-x-visible no-scrollbar px-3 py-4">
        ...
        <!-- Rodapé: PWA (mobile) + faturamento / conquistas (painel infoprodutor) -->
        <div
            v-if="
                !page.props.customer_panel
                && (showText() || isMobileOpen)
                && (page.props.achievementsProgress || (isMobile && showText()))
            "
            class="mt-auto shrink-0"
        >
            <div v-if="isMobile && showText()" class="space-y-2 px-4 pb-2 pt-3 lg:hidden">
                <PwaInstallButton />
            </div>
            <div v-if="page.props.achievementsProgress" class="px-3 pb-4 pt-2">
