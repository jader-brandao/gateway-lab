...
import { Upload, Trash2, Send, Smartphone, Puzzle, Info } from 'lucide-vue-next';
...
<template>
    <div class="space-y-6">
        ...
        <section
            v-if="!plugin_enabled"
            class="overflow-hidden rounded-xl border border-amber-200 bg-amber-50/60 p-8 shadow-sm dark:border-amber-900/40 dark:bg-amber-950/20"
        >
            <div class="flex flex-col items-center gap-4 text-center sm:flex-row sm:items-start sm:text-left">
                <div class="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300">
                    <Smartphone class="h-7 w-7" />
                </div>
                <div class="flex-1">
                    <h2 class="text-lg font-semibold text-zinc-900 dark:text-white">Plugin PWA não instalado</h2>
                    <p class="mt-2 text-sm text-zinc-600 dark:text-zinc-300">
                        A configuração do app (PWA) e o envio de notificações push do painel ficam disponíveis após instalar o plugin <strong>PWA</strong>.
                        Sem ele, o painel não pode ser instalado como aplicativo no celular dos usuários e o cadastro de inscrições push fica desativado.
                    </p>
                    <div class="mt-5 flex flex-wrap items-center gap-3">
                        <Link
                            href="/plataforma/gerenciar-plugins"
                            class="inline-flex items-center gap-2 rounded-xl bg-[var(--color-primary)] px-4 py-2.5 text-sm font-medium text-white shadow-sm transition hover:opacity-90"
                        >
        ...
        <section
            v-if="plugin_enabled"
            class="rounded-xl border border-blue-200 bg-blue-50/60 p-4 text-sm text-blue-900 dark:border-blue-900/40 dark:bg-blue-950/30 dark:text-blue-100"
        >
            <div class="flex items-start gap-3">
                <Info class="mt-0.5 h-5 w-5 shrink-0" />
                <p>
                    Após salvar, peça aos usuários que estiverem com o painel aberto para fazer um <strong>recarregamento forçado</strong>
                    (Ctrl+Shift+R no desktop, ou fechar e reabrir o navegador no mobile). Quem já instalou o app na tela inicial precisa
                    <strong>desinstalar e reinstalar</strong> para ver o novo nome e ícone — comportamento padrão do Android e do iOS.
                </p>
            </div>
        </section>

        <section v-if="plugin_enabled" class="overflow-hidden rounded-xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-700 dark:bg-zinc-800/50">
