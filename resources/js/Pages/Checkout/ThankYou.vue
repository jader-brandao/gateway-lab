    ...
    <div class="min-h-screen flex flex-col items-center justify-center bg-zinc-50 px-4">
        <div class="w-full max-w-md rounded-2xl border border-zinc-200 bg-white p-8 shadow-sm text-center">
            ...
            <a
                v-if="show_button"
                :href="redirect_url"
                class="mt-6 inline-flex w-full justify-center rounded-xl bg-[var(--color-primary,#8A2BE2)] px-4 py-3 text-sm font-semibold text-white shadow-sm hover:opacity-90 transition-opacity"
            >
                {{ redirect_label }}
            </a>
        </div>
